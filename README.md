...
画像はビルド時に WebP へ変換して配信し、ブラウザに JavaScript フレームワークは配信しない。
...
`images/<slug>/` に置き、記事からは絶対パスで参照する。拡張子は元のままでよい。

```markdown
![](/images/mcp-authorization/flow.png)
```

ビルド時に WebP へ変換され、配信されるのは変換後のファイル。元の PNG/GIF は `images/` に残る。
公開記事から参照されていない画像は配信対象に含まれない。
...
配信時のキャッシュで最大 1 日は古い版が表示される（`packages/web/public/_headers`）。
すぐ反映したいときはファイル名を変えて記事の参照も更新する。
