...
1. **GitHubアカウント** - 記事の管理にGitHubを使用します
2. **Auth Vaultアカウント** - GitHubアカウントでログインして作成します
3. **GitHubリポジトリの連携** - 記事を管理するリポジトリを1つ連携します

:::message alert
リポジトリの連携は **1アカウントにつき1つ** のみです。複数のリポジトリを同時に連携することはできません。連携するリポジトリを変更したい場合は、設定画面から現在の連携を解除してから、新しいリポジトリを連携してください。
:::
...
トップページ右上の「ログイン」ボタンをクリックし、GitHubアカウントで認証を行います。
...
いいえ、1アカウントにつき1つのリポジトリのみ連携可能です。連携先を変更したい場合は、設定画面から現在の連携を解除してから、新しいリポジトリを連携してください。
